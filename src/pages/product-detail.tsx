...
import { Product, Review, Inventory } from "@/shared/schema";
...
export default function ProductDetail() {
  ...
  const {
    ...
    isError,
      ...
      queryFn: async () => await ProductsAPI.getProduct(id),      
      enabled: !isNaN(id),
  });
  ...
  useEffect(() => {
    // Only add to recently viewed if product id is valid and not already tracked
    if (id && !isNaN(id) && !recentlyViewedTracked.current) {
  ...
  }, [id]); // Removed addToRecentlyViewed from dependencies
  ...
  return (
    <div className="bg-white">
      <div className="container mx-auto px-4 py-8">
          ...
          <div>
            ...
            <div className="flex gap-4 mb-8">
              <Button 
                ...
                disabled={Boolean(!selectedSize || isInventoryLoading || addingToCart || (selectedSize && inventory?.find(item => item.size === selectedSize)?.quantity === 0))}
              >
                {addingToCart ? (
                  <>
        ...
        {/* You May Also Like */}
        <ProductRecommendations productId={id} className="mt-16" />
      </div>
    </div>
  );
}
