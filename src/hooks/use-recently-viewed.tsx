...
import { Product, RecentlyViewed } from "@shared/schema";
...
export function RecentlyViewedProvider({ children }: { children: ReactNode }) {
  ...
  const { user } = useAuth();
    ...
    const { data: apiItems, isLoading } = useQuery <Product[]>({
    queryKey: ["/api/recently-viewed"],
    queryFn: () => RecentlyViewedAPI.getRecentlyViewedByUser(user.id),
    enabled: !!user,
  });
  ...
  const addMutation = useMutation({
    mutationFn: async (productId: number) => {
      ...
      if (user) {
        await RecentlyViewedAPI.getRecentlyViewedByUser(user.id);
