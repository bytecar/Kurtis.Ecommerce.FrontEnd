...
export function ProductCard({ product, onQuickView }: ProductCardProps) {
  ...
  return (
    <Card className="group overflow-hidden border-0 shadow-none relative h-full flex flex-col">
      <div className="relative overflow-hidden rounded-lg grow-on-hover">
        ...
        {user && (
          <Button
            variant="ghost"
            size="icon"
            className={`absolute top-2 right-2 z-10 bg-white/90 opacity-0 group-hover:opacity-100 transition-opacity ${isInWishlist(product.id) ? 'text-destructive hover:text-destructive' : 'text-gray-500 hover:text-gray-700'}`}
        ...
        <Link href={`/product/${product.id}`}>
          <a className="block">
            <img 
              ...
              alt={product.name}
              className="h-64 md:h-72 w-full object-cover transition-all"
            />
          </a>
        </Link>
      </div>
